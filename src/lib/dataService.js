...
export async function fetchOrders() {
  ...
  const full = orders.map((order) => {
    ...
    return {
      ...
      date: order.date, deliveryDate: order.delivery_date, shippedDate: order.shipped_date || null, actualDeliveryDate: order.actual_delivery_date || null, alertDays: order.alert_days,
...
function buildOrderPayload(order) {
  return {
    ...
    date: order.date, delivery_date: order.deliveryDate || null, shipped_date: order.shippedDate || null, actual_delivery_date: order.actualDeliveryDate || null, alert_days: order.alertDays || 7,
    ...
    installments_granted: order.installmentsGranted || false,
