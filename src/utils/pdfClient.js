...
import { getAllArticles, artPieceCount, orderTotal } from '../data/mockData.js'
  ...
  const pricingBlock = (() => {
    if (order.pricingMode === 'kit') {
      return order.kits.map(kit => {
        ...
        const kitTotal  = (kit.price || 0) * kitPieces
        return `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #e8e0d0;">
            <div>
              ...
              <div style="font-size:10px;color:#8a9ab5;margin-top:2px;letter-spacing:1px;">
                ${kit.articles.map(a => a.description).join(' + ')}
              </div>
            </div>
            <div style="text-align:right;">
              <div style="font-size:10px;color:#8a9ab5;letter-spacing:2px;">PREZZO KIT</div>
              <div style="font-family:'Cormorant Garamond',serif;font-size:22px;color:#c4623a;">€ ${(kit.price || 0).toFixed(2)}</div>
      ...
      return articles.map(a => {
        ...
        const tot = (a.price || 0) * pz
        return `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #e8e0d0;">
            <div>
              <div style="font-family:'Cormorant Garamond',serif;font-size:18px;color:#1a2744;">${a.description}</div>
              <div style="font-size:10px;color:#8a9ab5;margin-top:2px;letter-spacing:1px;">${a.category} · ${a.line} · ${a.color}</div>
            </div>
            <div style="text-align:right;">
              <div style="font-size:10px;color:#8a9ab5;letter-spacing:2px;">PREZZO UNITARIO</div>
              <div style="font-family:'Cormorant Garamond',serif;font-size:22px;color:#c4623a;">€ ${(a.price || 0).toFixed(2)}</div>
  ...
  const articleRows = articles.map(art => {
    ...
    const adultSection = hasAdult ? `
      <tr style="background:#f0f4f8;">
        ...
        ${ADULT_SIZES.map(sz => `<td style="padding:6px;text-align:center;font-size:9px;letter-spacing:1px;color:#8a9ab5;border:1px solid #e0d8cc;">${sz}</td>`).join('')}
    ...
    const kidsSection = hasKids ? `
      <tr style="background:#f4f0f8;">
        ...
        ${KIDS_SIZES.map(sz => `<td style="padding:6px;text-align:center;font-size:9px;letter-spacing:1px;color:#8a9ab5;border:1px solid #e0d8cc;">${sz}</td>`).join('')}
        <td style="padding:6px;text-align:center;font-size:9px;color:#8a9ab5;border:1px solid #e0d8cc;">TOT</td>
      </tr>
      <tr>
        <td style="padding:8px 10px;border:1px solid #e0d8cc;font-size:11px;color:#8a9ab5;">pz</td>
    ...
    return `
      <div style="margin-bottom:28px;page-break-inside:avoid;">
        <div style="display:flex;gap:16px;align-items:flex-start;margin-bottom:10px;flex-wrap:wrap;">
          <div>
            <div style="font-family:'Cormorant Garamond',serif;font-size:20px;color:#1a2744;letter-spacing:1px;">${art.description}</div>
          ...
          <div style="margin-left:auto;text-align:right;">
            ...
            <div style="font-size:13px;font-weight:700;color:#c4623a;letter-spacing:1px;">${art.color}</div>
          </div>
        </div>
        ...
        <div style="text-align:right;margin-top:6px;font-size:11px;color:#8a9ab5;">
          Totale pezzi: <strong style="color:#1a2744;">${grandTotal}</strong>
        </div>
      </div>
    `
  }).join('')
...
<body>
  ...
  <div style="background:#f8f5f0;padding:20px 40px;border-bottom:2px solid #e0d8cc;">
    <div style="display:flex;gap:40px;flex-wrap:wrap;">
      <div><div style="font-size:9px;letter-spacing:3px;color:#8a9ab5;margin-bottom:4px;">CLUB</div>
        <div style="font-family:'Cormorant Garamond',serif;font-size:22px;color:#1a2744;">${order.client}</div></div>
      <div><div style="font-size:9px;letter-spacing:3px;color:#8a9ab5;margin-bottom:4px;">DATA ORDINE</div>
      ...
      <div><div style="font-size:9px;letter-spacing:3px;color:#8a9ab5;margin-bottom:4px;">TOTALE PEZZI</div>
        <div style="font-family:'Cormorant Garamond',serif;font-size:28px;color:#c4623a;">${order.pieces}</div></div>
      <div><div style="font-size:9px;letter-spacing:3px;color:#8a9ab5;margin-bottom:4px;">STATO</div>
        <div style="font-size:12px;font-weight:700;letter-spacing:2px;">${order.status}</div></div>
    </div>
  ...
  <div style="margin:0 40px 28px;background:#f8f5f0;border:1px solid #e0d8cc;border-radius:10px;padding:22px 28px;page-break-inside:avoid;">
    <div style="font-size:9px;letter-spacing:4px;color:#8a9ab5;margin-bottom:16px;">
      ${order.pricingMode === 'kit' ? 'COMPOSIZIONE KIT E PREZZI' : 'PREZZI PER ARTICOLO'}
    </div>
    ${pricingBlock}
  ...
  <div style="margin:0 40px 28px;padding:22px 28px;border:1px solid #e0d8cc;border-radius:10px;page-break-inside:avoid;">
    ...
    <div style="font-size:11px;color:#555;line-height:1.8;">
      ...
      <p style="margin-bottom:8px;"><strong>2. Conferma dell'Ordine</strong> — L'ordine si considera confermato esclusivamente al verificarsi congiunto di: firma del presente documento, approvazione grafica definitiva, versamento dell'acconto previsto.</p>
      <p style="margin-bottom:8px;"><strong>3. Produzione e Tempistiche</strong> — I tempi decorrono dalla data di conferma definitiva. Le tempistiche concordate sono indicative e possono subire variazioni per cause di forza maggiore.</p>
      <p style="margin-bottom:8px;"><strong>4. Prodotti Personalizzati</strong> — Non è previsto diritto di recesso. Non è possibile annullare l'ordine dopo l'avvio della produzione. I prodotti non sono accettati a reso per errori di taglia comunicati dal cliente.</p>
