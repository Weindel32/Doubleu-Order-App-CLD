...
function artSizeTotal(art) {
  const ad = ADULT_SIZES.reduce((s, sz) => s + (art.sizes?.adult?.[sz] || 0), 0)
  const ki = KIDS_SIZES.reduce((s, sz) => s + (art.sizes?.kids?.[sz] || 0), 0)
  return ad + ki
}

function sizeTableHtml(art) {
  const adTotal = ADULT_SIZES.reduce((s, sz) => s + (art.sizes?.adult?.[sz] || 0), 0)
  const kiTotal = KIDS_SIZES.reduce((s, sz) => s + (art.sizes?.kids?.[sz] || 0), 0)
  if (adTotal + kiTotal === 0) return ''

  const cellStyle = 'padding:5px 8px;text-align:center;border:1px solid #e0d8cc;font-size:11px;'
  const headStyle = cellStyle + 'background:#f0ece4;font-size:9px;letter-spacing:1px;color:#8a9ab5;font-weight:600;'
  const valStyle  = cellStyle + 'font-family:"Cormorant Garamond",serif;font-size:15px;color:#1a2744;'
  const totStyle  = cellStyle + 'background:#fff7f0;font-family:"Cormorant Garamond",serif;font-size:15px;color:#c4623a;font-weight:700;'

  const adRow = adTotal > 0 ? `
    <tr>
      <td style="${headStyle}color:#1a2744;">ADULTO</td>
      ${ADULT_SIZES.map(sz => `<td style="${headStyle}">${sz}</td>`).join('')}
      <td style="${headStyle}">—</td><td style="${headStyle}">—</td><td style="${headStyle}">—</td><td style="${headStyle}">—</td><td style="${headStyle}">—</td>
      <td style="${totStyle}">TOT</td>
    </tr>
    <tr>
      <td style="${cellStyle}"></td>
      ${ADULT_SIZES.map(sz => `<td style="${(art.sizes?.adult?.[sz]||0)>0?valStyle:cellStyle+'color:#ccc;'}">${art.sizes?.adult?.[sz]||0}</td>`).join('')}
      <td style="${cellStyle}"></td><td style="${cellStyle}"></td><td style="${cellStyle}"></td><td style="${cellStyle}"></td><td style="${cellStyle}"></td>
      <td style="${totStyle}">${adTotal}</td>
    </tr>` : ''

  const kiRow = kiTotal > 0 ? `
    <tr>
      <td style="${headStyle}color:#1a2744;">BAMBINO</td>
      ${KIDS_SIZES.map(sz => `<td style="${headStyle}">${sz}</td>`).join('')}
      <td style="${totStyle}">TOT</td>
    </tr>
    <tr>
      <td style="${cellStyle}"></td>
      ${KIDS_SIZES.map(sz => `<td style="${(art.sizes?.kids?.[sz]||0)>0?valStyle:cellStyle+'color:#ccc;'}">${art.sizes?.kids?.[sz]||0}</td>`).join('')}
      <td style="${totStyle}">${kiTotal}</td>
    </tr>` : ''

  return `<div style="margin-top:10px;overflow-x:auto;">
    <table style="border-collapse:collapse;font-family:'Josefin Sans',sans-serif;">
      ${adRow}${kiRow}
    </table>
    <div style="margin-top:4px;font-size:9px;letter-spacing:2px;color:#c4623a;">TOTALE PEZZI: ${adTotal + kiTotal}</div>
  </div>`
}
      ...
      return articles.map(a => {
        ...
        return `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #e8e0d0;">
            ...
            <div style="text-align:right;">
              ${qty ? `
              <div style="font-size:10px;color:#8a9ab5;letter-spacing:2px;">PREZZO × ${sizesQty > 0 ? 'PEZZI DA TAGLIE' : 'QUANTITÀ STIMATA'}</div>
...
<body>
  ...
  <div style="padding:28px 40px;">
    ...
    ${!anyHasSizes ? `<div style="font-size:10px;color:#b8965a;letter-spacing:1px;margin-bottom:16px;padding:8px 12px;background:#fff7f0;border-radius:4px;border-left:3px solid #c4623a;">
  ...
  <div style="margin:0 40px 40px;padding:22px 28px;border:1px solid #e0d8cc;border-radius:10px;page-break-inside:avoid;">
    ...
    <div style="font-size:11px;color:#555;line-height:1.8;">
      ...
      ${!anyHasSizes ? `<p style="margin-bottom:0;"><strong>Prodotti personalizzati</strong> — I prodotti sono realizzati su misura. Le taglie definitive saranno indicate in fase di conferma ordine.</p>` : ''}
    </div>
  </div>
