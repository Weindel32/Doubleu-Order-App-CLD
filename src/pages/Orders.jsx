...
import { GOLD, MUTED, CREAM, CLAY, NAVY, GREEN, BORDER } from '../tokens.js'
import { s, badgeStyle, btnStyle, btnGoldStyle, ORDER_STATUSES, STATUS_COLORS } from '../tokens.js'
...
function StatusSelector({ order, onStatusChange }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        <span style={{ ...badgeStyle(order.status), cursor: 'pointer' }}>
          {saving ? '...' : order.status} {!saving && '▾'}
        </span>
      </div>
      {open && (
        <div style={{
          position: 'absolute', top: '100%', left: 0, zIndex: 100, marginTop: 4,
          background: '#1e2d50', border: `1px solid ${BORDER}`, borderRadius: 8,
          padding: 6, minWidth: 180, boxShadow: '0 8px 24px rgba(0,0,0,0.4)',
        }}>
          {ORDER_STATUSES.map(st => {
            const sc = STATUS_COLORS[st] || STATUS_COLORS['PREVENTIVO']
            return (
              <div key={st} onClick={() => handleSelect(st)} style={{
                padding: '8px 12px', borderRadius: 4, cursor: 'pointer',
                fontSize: 9, letterSpacing: 2, fontWeight: 700,
                color: sc.color, background: order.status === st ? sc.bg : 'transparent',
    ...
    <div>
      {order.payments.map(p => (
        <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          <div
            onClick={() => saving !== p.id && toggle(p)}
            style={{
              width: 14, height: 14, borderRadius: '50%', cursor: 'pointer', flexShrink: 0,
              border: `1.5px solid ${p.paid ? GREEN : GOLD}`,
              background: p.paid ? GREEN : 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'all 0.2s', opacity: saving === p.id ? 0.5 : 1,
            }}
          >
            {p.paid && <span style={{ color: 'white', fontSize: 8, lineHeight: 1 }}>✓</span>}
          </div>
          <span style={{ fontSize: 9, color: p.paid ? GREEN : GOLD, letterSpacing: 0.5 }}>
            € {(p.amount || 0).toLocaleString('it-IT', { maximumFractionDigits: 0 })} {p.type}
          </span>
        </div>
      ))}
      {/* Progress bar */}
...
export default function Orders({ orders, setView, setEditOrder, onDelete, onOrdersChange }) {
  ...
  const handleStatusChange = (orderId, newStatus) => {
    onOrdersChange(orders.map(o => o.id === orderId ? { ...o, status: newStatus } : o))
  }

  const handlePaymentToggle = (orderId, paymentId, newPaid) => {
    onOrdersChange(orders.map(o => o.id !== orderId ? o : {
      ...o,
      payments: o.payments.map(p => p.id === paymentId ? { ...p, paid: newPaid } : p)
    }))
  }
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {filters.map(f => (
            <button key={f} style={{
              ...
              background: filter === f ? GOLD : 'rgba(255,255,255,0.05)',
              color: filter === f ? NAVY : MUTED, fontWeight: filter === f ? 700 : 400,
              ...
              return (
                <tr key={o.id} style={{ background: alert ? 'rgba(196,98,58,0.04)' : 'transparent' }}>
                  ...
                  <td style={{ ...s.td, fontSize: 11, color: days !== null && days <= 7 && o.status !== 'CONSEGNATO' ? CLAY : MUTED }}>
                    {o.deliveryDate || '—'}
                    {days !== null && o.status !== 'CONSEGNATO' && (
                      <div style={{ fontSize: 9, marginTop: 2 }}>
                        {days < 0 ? `scad. ${Math.abs(days)}gg` : days === 0 ? 'oggi' : `${days}gg`}
                      </div>
                    )}
                  </td>
                  <td style={s.td}>
                    <StatusSelector order={o} onStatusChange={handleStatusChange} />
                  </td>
                  ...
                  <td style={s.td}>
                    <PaymentQuick order={o} onPaymentToggle={handlePaymentToggle} />
                  </td>
                  <td style={s.td}>
                    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                      <button style={{ ...btnGoldStyle, padding: '4px 8px', fontSize: 8 }}
                        onClick={() => { setEditOrder(o); setView('new') }}>Apri</button>
                      <button style={{ padding: '4px 8px', fontSize: 8, border: '1px solid rgba(196,98,58,0.4)', background: 'rgba(196,98,58,0.08)', color: CLAY, borderRadius: 3, cursor: 'pointer' }}
                        onClick={() => openPDF(generateProductionPDF, o)}>Prod.</button>
                      <button style={{ padding: '4px 8px', fontSize: 8, border: `1px solid rgba(184,150,90,0.3)`, background: 'rgba(184,150,90,0.06)', color: GOLD, borderRadius: 3, cursor: 'pointer' }}
                        onClick={() => openPDF(generateClientPDF, o)}>Cliente</button>
                      <button style={{ padding: '4px 8px', fontSize: 8, border: '1px solid rgba(122,174,232,0.3)', background: 'rgba(122,174,232,0.06)', color: '#7aaee8', borderRadius: 3, cursor: 'pointer' }}
                        onClick={() => exportSizesCSV(o)}>CSV</button>
