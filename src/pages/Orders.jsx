...
export default function Orders({ setView, setEditOrder }) {
    ...
    <div>
      <div style={s.topBar}>
        <div>
          ...
          <div style={s.pageSub}>{MOCK_ORDERS.length} ordini in archivio</div>
        </div>
        <div style={{display:'flex',gap:10,alignItems:'center'}}>
          <button style={{...btnStyle(false),color:'#7aaee8',border:'1px solid rgba(122,174,232,0.3)',background:'rgba(122,174,232,0.06)',padding:'9px 18px'}}
            onClick={()=>exportAllOrdersCSV(MOCK_ORDERS)}>
            ↓ Export CSV
          </button>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,gap:16,flexWrap:'wrap'}}>
        <input
          style={{...s.input,width:260,padding:'9px 14px'}}
        ...
        <div style={{fontSize:10,color:MUTED,letterSpacing:2,whiteSpace:'nowrap'}}>{filtered.length} risultati</div>
      </div>
            ...
            return (
              <tr key={o.id} style={{background:alert?'rgba(196,98,58,0.04)':'transparent'}}>
                ...
                <td style={{...s.td,fontSize:11,color:days!==null&&days<=7&&!['CONSEGNATO'].includes(o.status)?CLAY:MUTED}}>
                  {o.deliveryDate||'—'}
                  {days!==null&&!['CONSEGNATO'].includes(o.status)&&(
                    <div style={{fontSize:9,marginTop:2}}>{days<0?`scad. ${Math.abs(days)}gg`:days===0?'oggi':`${days}gg`}</div>
                  )}
                </td>
