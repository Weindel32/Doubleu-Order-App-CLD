...
import { MOCK_ORDERS, getAllArticles, artPieceCount } from '../data/mockData.js'
...
function buildStats() {
  ...
  // Pieces by category across all confirmed orders
  const byCategory = {}
  ...
  const byStatus = {}
  allOrders.forEach(o => {
    byStatus[o.status] = (byStatus[o.status] || 0) + o.pieces
  })

  const totalPieces = confirmed.reduce((s, o) => s + o.pieces, 0)

  return { byCategory, byLine, byStatus, totalPieces }
}
            ...
            <div key={label}>
              ...
              <div style={{ height: 6, background: 'rgba(255,255,255,0.06)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{
                  height: '100%',
                  width: `${pct}%`,
                  background: colorFn ? colorFn(label) : `linear-gradient(90deg, ${GOLD}, ${CLAY})`,
                  borderRadius: 3,
...
function DonutChart({ data, title }) {
  ...
  const arcs = entries.map(([label, val], i) => {
    ...
    return { label, val, pct, color: colors[i % colors.length], d: `M ${cx} ${cy} L ${x1} ${y1} A ${r} ${r} 0 ${largeArc} 1 ${x2} ${y2} Z` }
  })
    ...
    <div style={{ ...s.card, marginBottom: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 28, alignItems: 'center', flexWrap: 'wrap' }}>
        <svg width="140" height="140" style={{ flexShrink: 0 }}>
          ...
          <circle cx={cx} cy={cy} r={r - strokeW} fill={NAVY} />
          <text x={cx} y={cy - 6} textAnchor="middle" fill={CREAM} fontSize="20" fontFamily="Cormorant Garamond, serif" fontWeight="300">
            {total}
          </text>
          <text x={cx} y={cy + 10} textAnchor="middle" fill={MUTED} fontSize="8" fontFamily="Josefin Sans, sans-serif" letterSpacing="2">
            PEZZI
          </text>
        </svg>
...
export default function Analytics() {
  const { byCategory, byLine, byStatus, totalPieces } = buildStats()
  ...
  return (
    <div>
      ...
      <div style={s.grid4}>
        {[
          ...
          <div key={item.label} style={s.statCard(false)}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: item.value.toString().length > 4 ? 22 : 34, fontWeight: 300, color: CREAM, lineHeight: 1 }}>
              {item.value}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <BarChart   data={byCategory} title="Distribuzione per Referenza" colorFn={l => catColorMap[l] || MUTED} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <BarChart data={byLine}   title="Pezzi per Linea" colorFn={l => lineColorMap[l] || GOLD} />
        <BarChart data={byStatus} title="Pezzi per Stato Ordine" colorFn={() => `linear-gradient(90deg, ${GOLD}, rgba(184,150,90,0.4))`} />
      </div>

      {/* Table breakdown */}
      <div style={s.divider} />
      <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: CREAM, letterSpacing: 2, marginBottom: 20 }}>
        Breakdown Completo per Referenza
      </div>
      <table style={s.table}>
        <thead>
          <tr>
            {['Referenza', 'Linea', 'Categoria', 'Ordini', 'Pezzi Totali', '% sul totale'].map(h => (
        ...
        <tbody>
          {(() => {
            const rows = {}
            MOCK_ORDERS.filter(o => o.status !== 'PREVENTIVO').forEach(order => {
              getAllArticles(order).forEach(art => {
            ...
            return Object.values(rows)
              ...
              .map(row => (
                <tr key={row.description}>
                  ...
                  <td style={{ ...s.td, color: MUTED, fontSize: 11, letterSpacing: 1 }}>{row.line}</td>
                  <td style={s.td}>
                    <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 2, fontSize: 9, letterSpacing: 2, background: `${catColorMap[row.category]}22`, color: catColorMap[row.category] || GOLD, border: `1px solid ${catColorMap[row.category]}44` }}>
                      {row.category}
                    </span>
                  </td>
                  ...
                  <td style={s.td}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ flex: 1, height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 2 }}>
                        <div style={{ height: '100%', width: `${Math.round(row.pieces / totalPieces * 100)}%`, background: catColorMap[row.category] || GOLD, borderRadius: 2 }} />
                      </div>
