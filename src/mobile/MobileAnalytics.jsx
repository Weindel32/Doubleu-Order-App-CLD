  ...
  return (
    <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '16px', marginBottom: 20, position: 'relative' }} ref={containerRef}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 11, letterSpacing: 3, fontWeight: 600, color: GOLD, textTransform: 'uppercase' }}>Fatturato Mensile</div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          {years.map(year => {
            const st = getStyle(year)
            return (
              <span key={year} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, color: MUTED, letterSpacing: 2 }}>
        ...
        {MONTHS.map((m, i) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontSize: 10, fontWeight: 600, color: MUTED, letterSpacing: 1, textTransform: 'uppercase' }}>{m}</span>
            </div>
          )
        })}
      </div>
  ...
  if (orders.length === 0) {
    return (
      <div style={{ padding: '60px 20px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, letterSpacing: 1 }}>I grafici appariranno una volta inseriti gli ordini</div>
      </div>
    )
  }
    ...
    <div style={{ padding: '20px 16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
        <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '16px' }}>
          <div style={{ fontSize: 11, letterSpacing: 2, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginBottom: 4 }}>Fatturato {currentYear}</div>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, color: GOLD, lineHeight: 1 }}>{fmt(currentYearRevenue)}</div>
        </div>
        <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '16px' }}>
          <div style={{ fontSize: 11, letterSpacing: 2, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginBottom: 4 }}>Pezzi Prodotti</div>
        ...
        <div style={{ background: SURFACE, border: `1px solid rgba(196,98,58,0.35)`, borderRadius: 10, padding: '16px', gridColumn: '1 / -1' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            ...
            {totalPieces > 0 && <div style={{ fontSize: 12, fontWeight: 600, color: MUTED }}>{Math.round(totalOmaggio / totalPieces * 100)}% del totale prodotto</div>}
          </div>
        </div>
      </div>
        ...
        <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ display: 'flex' }}>
            <div style={{ flex: 1, padding: '16px', borderRight: `1px solid rgba(184,150,90,0.12)` }}>
              <div style={{ fontSize: 11, letterSpacing: 2, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginBottom: 4 }}>Istituzionale</div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: GOLD }}>{fmt(istituzionale)}</div>
              <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, marginTop: 2 }}>{pctIst}%</div>
            </div>
            <div style={{ flex: 1, padding: '16px' }}>
              <div style={{ fontSize: 11, letterSpacing: 2, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginBottom: 4 }}>Soci / Shop</div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: CLAY }}>{fmt(sociShop)}</div>
              <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, marginTop: 2 }}>{pctSoci}%</div>
            </div>
          </div>
          ...
          <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '16px' }}>
            <div style={{ display: 'flex', marginBottom: 14 }}>
              <div style={{ flex: 1, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 11, letterSpacing: 1, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginTop: 4 }}>Convertiti</div>
              </div>
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, color: '#ef4444', lineHeight: 1 }}>{lostCount}</div>
                <div style={{ fontSize: 11, letterSpacing: 1, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginTop: 4 }}>Persi</div>
              </div>
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, color: CLAY, lineHeight: 1 }}>{openCount}</div>
                <div style={{ fontSize: 11, letterSpacing: 1, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginTop: 4 }}>Aperti</div>
              </div>
            </div>
            {decidedCount > 0 && (
              <>
                ...
                <div style={{ fontSize: 11, letterSpacing: 1, fontWeight: 600, color: MUTED, marginTop: 6 }}>tasso di conversione sui preventivi decisi</div>
              </>
            )}
            {lossReasonRows.length > 0 && (
              <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid rgba(255,255,255,0.06)` }}>
                <div style={{ fontSize: 11, letterSpacing: 2, fontWeight: 600, color: MUTED, textTransform: 'uppercase', marginBottom: 10 }}>Motivi di perdita</div>
