...
function NewClientForm({ onSave, onCancel }) {
  const [fields, setFields] = useState({ name: '', contact: '', phone: '', email: '', city: '', fiscal_code: '' })
  ...
  const handleSave = async () => {
    ...
    const ok = await onSave(name.trim(), {
      ...
      fiscal_code: rest.fiscal_code.trim().toUpperCase() || null,
    })
    setSaving(false)
    ...
    <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '20px 16px', marginBottom: 20 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <input
          ...
          onFocus={() => setFocused('fiscal_code')}
          onBlur={() => setFocused(null)}
          placeholder="per ASD senza P.IVA"
          style={inputStyle(focused === 'fiscal_code')}
...
function mChip(active, tc) {
  return {
    padding: '7px 12px', borderRadius: 6, fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase',
...
export default function MobileClients({ clients, orders, onSelectOrder, onUpsertClient }) {
  ...
  const filtered = enriched.filter(c => {
    ...
    if (q) {
      const hay = [c.name, c.city, c.country, c.email, c.vat_number, c.fiscal_code].filter(Boolean).join(' ').toLowerCase()
