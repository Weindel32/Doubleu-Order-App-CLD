...
import { MOCK_ORDERS, daysUntilDelivery, needsAlert, paymentSummary, orderTotal } from '../data/mockData.js'

export default function AlertsPanel({ setView, setEditOrder }) {
  const alerts = MOCK_ORDERS.filter(o => needsAlert(o))
    .sort((a, b) => (daysUntilDelivery(a) ?? 999) - (daysUntilDelivery(b) ?? 999))

  const pendingPayments = MOCK_ORDERS.filter(o =>
    o.status !== 'PREVENTIVO' &&
    (o.payments || []).some(p => !p.paid)
  )

  if (alerts.length === 0 && pendingPayments.length === 0) return null

  const urgencyColor = (days) => {
    if (days < 0)  return '#ef4444'   // scaduto
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: MUTED, marginTop: 1, letterSpacing: 1 }}>{order.id} · {order.deliveryDate}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color, letterSpacing: 1 }}>{urgencyLabel(days)}</div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {pendingPayments.map(order => {
              ...
              const pendingList = order.payments.filter(p => !p.paid)
              return (
                <div key={order.id} style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '10px 14px',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div>
                      <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 16, color: CREAM }}>{order.client}</div>
                      <div style={{ fontSize: 10, color: MUTED, marginTop: 1, letterSpacing: 1 }}>
                        {order.id} · {pendingList.map(p => p.note).join(', ')}
                      </div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
